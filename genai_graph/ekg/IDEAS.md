...
Until now, the sub-graphs were created from Pydantic model generated by BAML and content stored in a KV store (through a abstract GraphFactory specialisation:  KvStoreBackedSubgraphFactory - see genai_graph/core/subgraph_factories.py )
We want now to have the ability to  create sub-graphs from unormalized tables (typically Excel exports from a CRM or other Saas products ).  To do so, I've created a new abstract GraphFactory class of type TableBackedSubgraphFactory.
Concrete implementation o these factories have been developped for the given use case, and configuredin the YMl file : config/ekg.yaml 
I want you to continue the job and connect the dot.
The use case is that command: ' cli kg delete -f ; cli kg create --config simple'. It should, as today, create a subgraph around 'ReviewedOpportunity'.  But it should also create SOME elements of graph of type CrmExtract, that is defined (but maube wrongly) in schema/crm_export.py.
The created subgraph contains 
- node of type 'Opportunity' that has to be merged with the one comming from ReviewedOpportunity
- Node 'person' that has  to be created, and connected by a relationship to Opportunity
- Node 'WinLoss' wich is an extra_class of nodes associeted to Opporunity (see genai_graph/ekg/schema/common_nodes.py).  You need to change the logic to handle such "ExtraFields" (in genai_graph/core/extra_fields_utils.py): Except for hard-coded one like 'metadata', the value should be taken from other defined table based subgraphs.
For that reason, you should create these subgraph first. 



continue (and possibly correct) impmementation of /home/tcl/prj/genai-graph/genai_graph/ekg/schema/crm_export.py .



- check with : uv run cli kg create --config "simple" ; uv run cli kg schema
